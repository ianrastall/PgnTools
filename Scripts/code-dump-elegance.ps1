[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\Elegance_Context.txt",
    [string]$ExtraManifestPath = "",
    [string[]]$Tags = @("ELEGANCE"),
    [switch]$TaggedOnly,
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-elegance.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Elegance")
