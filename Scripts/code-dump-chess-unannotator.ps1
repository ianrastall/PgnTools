[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\ChessUnannotator_Context.txt",
    [string]$ExtraManifestPath = "",
    [string[]]$Tags = @("CHESSUNANNOTATOR"),
    [switch]$TaggedOnly,
...
$manualFiles = @(
    "Docs\ChessUnannotatorService.md"
    "PgnTools\Services\ChessUnannotatorService.cs"
    "PgnTools\ViewModels\Tools\ChessUnannotatorViewModel.cs"
    "PgnTools\Views\Tools\ChessUnannotatorPage.xaml"
    "PgnTools\Views\Tools\ChessUnannotatorPage.xaml.cs"
...
$autoPatterns = @(
    "PgnTools\Services\*ChessUnannotator*.cs"
    "PgnTools\ViewModels\Tools\*ChessUnannotator*.cs"
    "PgnTools\Views\Tools\*ChessUnannotator*.xaml"
    "PgnTools\Views\Tools\*ChessUnannotator*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-chess-unannotator.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Chess Unannotator")
