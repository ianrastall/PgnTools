[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\Chesscom_Context.txt",
...
$manualFiles = @(
    ...
    "PgnTools\Services\ChesscomDownloaderService.cs"
    "PgnTools\ViewModels\Tools\ChesscomDownloaderViewModel.cs"
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-chesscom.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Chess.com")
