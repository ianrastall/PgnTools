[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\Chesscom_Monthly_Context.txt",
    [string]$ExtraManifestPath = "",
    [string[]]$Tags = @("CHESSCOM"),
    [switch]$TaggedOnly,
...
$manualFiles = @(
    "Docs\ChesscomMonthlyCrawler.md"
    "Docs\ChesscomDownloaderService.md"
    "PgnTools\Services\ChesscomMonthlyDownloaderService.cs"
    "PgnTools\ViewModels\Tools\ChesscomMonthlyDownloaderViewModel.cs"
    "PgnTools\ViewModels\Tools\ChesscomToolsViewModel.cs"
    "PgnTools\Views\Tools\ChesscomMonthlyDownloaderView.xaml"
    "PgnTools\Views\Tools\ChesscomMonthlyDownloaderView.xaml.cs"
    "PgnTools\Views\Tools\ChesscomDownloaderPage.xaml"
    "PgnTools\Views\Tools\ChesscomDownloaderPage.xaml.cs"
    "PgnTools\Helpers\FilePickerHelper.cs"
    "PgnTools\Helpers\FileValidationHelper.cs"
)

$autoPatterns = @(
    "PgnTools\Services\*Chesscom*.cs"
    "PgnTools\ViewModels\Tools\*Chesscom*.cs"
    "PgnTools\Views\Tools\*Chesscom*.xaml"
    "PgnTools\Views\Tools\*Chesscom*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-chesscom-monthly.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Chess.com Monthly Crawler")
