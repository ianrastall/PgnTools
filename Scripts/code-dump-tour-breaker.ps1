[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\TourBreaker_Context.txt",
    [string]$ExtraManifestPath = "",
    [string[]]$Tags = @("TOURBREAKER"),
    [switch]$TaggedOnly,
...
$manualFiles = @(
    "Docs\TourBreakerService.md"
    "PgnTools\Services\TourBreakerService.cs"
    "PgnTools\ViewModels\Tools\TourBreakerViewModel.cs"
    "PgnTools\Views\Tools\TourBreakerPage.xaml"
    "PgnTools\Views\Tools\TourBreakerPage.xaml.cs"
...
$autoPatterns = @(
    "PgnTools\Services\*TourBreaker*.cs"
    "PgnTools\ViewModels\Tools\*TourBreaker*.cs"
    "PgnTools\Views\Tools\*TourBreaker*.xaml"
    "PgnTools\Views\Tools\*TourBreaker*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-tour-breaker.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Tour Breaker")
