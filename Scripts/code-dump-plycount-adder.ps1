[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\PlycountAdder_Context.txt",
    [string]$ExtraManifestPath = "",
    [string[]]$Tags = @("PLYCOUNTADDER"),
    [switch]$TaggedOnly,
...
$manualFiles = @(
    "Docs\PlycountAdderService.md"
    "PgnTools\Services\IPlycountAdderService.cs"
    "PgnTools\Services\PlycountAdderService.cs"
    "PgnTools\ViewModels\Tools\PlycountAdderViewModel.cs"
    "PgnTools\Views\Tools\PlycountAdderPage.xaml"
    "PgnTools\Views\Tools\PlycountAdderPage.xaml.cs"
...
$autoPatterns = @(
    "PgnTools\Services\*Plycount*.cs"
    "PgnTools\ViewModels\Tools\*Plycount*.cs"
    "PgnTools\Views\Tools\*Plycount*.xaml"
    "PgnTools\Views\Tools\*Plycount*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-plycount-adder.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Ply Count Adder")
