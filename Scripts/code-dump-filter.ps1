[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\Filter_Context.txt",
...
$manualFiles = @(
    "Docs\PgnFilterService.md"
    "PgnTools\Services\PgnFilterService.cs"
    "PgnTools\ViewModels\Tools\FilterViewModel.cs"
    "PgnTools\Views\Tools\FilterPage.xaml"
    "PgnTools\Views\Tools\FilterPage.xaml.cs"
...
$autoPatterns = @(
    "PgnTools\Services\PgnFilter*.cs"
    "PgnTools\ViewModels\Tools\Filter*.cs"
    "PgnTools\Views\Tools\Filter*.xaml"
    "PgnTools\Views\Tools\Filter*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-filter.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Filter")
