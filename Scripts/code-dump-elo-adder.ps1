[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\EloAdder_Context.txt",
    [string]$ExtraManifestPath = "",
    [string[]]$Tags = @("ELOADDER"),
    [switch]$TaggedOnly,
...
$manualFiles = @(
    "Docs\EloAdderService.md"
    "PgnTools\Services\EloAdderService.cs"
    "PgnTools\Services\EmbeddedRatingsDatabase.cs"
    "PgnTools\ViewModels\Tools\EloAdderViewModel.cs"
    "PgnTools\Views\Tools\EloAdderPage.xaml"
    "PgnTools\Views\Tools\EloAdderPage.xaml.cs"
...
$autoPatterns = @(
    "PgnTools\Services\*EloAdder*.cs"
    "PgnTools\ViewModels\Tools\*EloAdder*.cs"
    "PgnTools\Views\Tools\*EloAdder*.xaml"
    "PgnTools\Views\Tools\*EloAdder*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-elo-adder.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: Elo Adder")
