[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\EcoTagger_Context.txt",
    [string]$ExtraManifestPath = "",
    [string[]]$Tags = @("ECOTAGGER"),
    [switch]$TaggedOnly,
...
$manualFiles = @(
    "Docs\EcoTaggerService.md"
    "PgnTools\Services\EcoTaggerService.cs"
    "PgnTools\ViewModels\Tools\EcoTaggerViewModel.cs"
    "PgnTools\Views\Tools\EcoTaggerPage.xaml"
    "PgnTools\Views\Tools\EcoTaggerPage.xaml.cs"
...
$autoPatterns = @(
    "PgnTools\Services\*EcoTagger*.cs"
    "PgnTools\ViewModels\Tools\*EcoTagger*.cs"
    "PgnTools\Views\Tools\*EcoTagger*.xaml"
    "PgnTools\Views\Tools\*EcoTagger*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-eco-tagger.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: ECO Tagger")
