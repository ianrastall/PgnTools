[CmdletBinding()]
param(
    [string]$OutputPath = "..\Context\PgnInfo_Context.txt",
...
$manualFiles = @(
    "Docs\PgnInfoService.md"
    "PgnTools\Services\PgnInfoService.cs"
    "PgnTools\ViewModels\Tools\PgnInfoViewModel.cs"
    "PgnTools\Views\Tools\PgnInfoPage.xaml"
    "PgnTools\Views\Tools\PgnInfoPage.xaml.cs"
...
$autoPatterns = @(
    "PgnTools\Services\*PgnInfo*.cs"
    "PgnTools\ViewModels\Tools\*PgnInfo*.cs"
    "PgnTools\Views\Tools\*PgnInfo*.xaml"
    "PgnTools\Views\Tools\*PgnInfo*.xaml.cs"
)
...
if ([string]::IsNullOrWhiteSpace($ExtraManifestPath)) {
    $ExtraManifestPath = Join-Path $ScriptLocation "code-dump-pgn-info.extra.txt"
}
...
try {
    $writer.WriteLine("# Context Dump: PGN Info")
